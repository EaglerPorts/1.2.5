...
public class FontRenderer {
	...
	protected int[] glyphTextureName = new int[256];
	...
	public FontRenderer(GameSettings gameSettingsIn, String s, RenderEngine textureManagerIn, boolean unicodeFlag) {
		...
		try {
			java.io.InputStream in = EagRuntime.getResourceStream("/font/glyph_sizes.bin");
			if (in != null) {
				in.read(this.glyphWidth);
				in.close();
			}
		} catch (Exception e) {
		}
	...
	private float func_181559_a(char parChar1) {
		...
		if (unicodeFlag && glyphWidth[parChar1] != 0) {
			return renderUnicodeChar(parChar1, false);
		}
		return this.renderDefaultChar(parChar1);
	}

	private void loadGlyphTexture(int page) {
		String path = String.format("/font/glyph_%02X.png", Integer.valueOf(page));
		ImageData img = ImageData.loadImageFile(EagRuntime.getResourceStream(path));
		if (img == null) {
			glyphTextureName[page] = 0;
			return;
		}
		glyphTextureName[page] = renderEngine.allocateAndSetupTexture(img);
	}
	...
	private float renderUnicodeChar(char ch, boolean italic) {
		...
		int page = ch / 256;
		if (glyphTextureName[page] == 0) {
			loadGlyphTexture(page);
		}
		int tex = glyphTextureName[page];
		if (tex == 0) {
			return 0.0F;
		}

		GL11.glBindTexture(RealOpenGLEnums.GL_TEXTURE_2D, tex);

		int start = glyphWidth[ch] >>> 4;
		int end = glyphWidth[ch] & 15;
		float u0 = (float) start;
		float u1 = (float) (end + 1);
		...
		float w = (u1 - u0) / 2.0F + 1.0F;
		float italicOffset = italic ? 1.0F : 0.0F;

		float u0f = ((ch & 15) * 16.0F + u0) / 256.0F;
		float v0f = ((ch & 255) / 16 * 16.0F) / 256.0F;
		float u1f = ((ch & 15) * 16.0F + u1 - 0.02F) / 256.0F;
		float v1f = (v0f + 15.98F / 256.0F);

		Tessellator tess = Tessellator.getInstance();
		WorldRenderer wr = tess.getWorldRenderer();

		wr.begin(Tessellator.GL_TRIANGLE_STRIP, VertexFormat.POSITION_TEX);
		wr.pos(this.posX + italicOffset, this.posY, 0.0F).tex(u0f, v0f).endVertex();
		wr.pos(this.posX - italicOffset, this.posY + 7.99F, 0.0F).tex(u0f, v1f).endVertex();
		wr.pos(this.posX + w + italicOffset, this.posY, 0.0F).tex(u1f, v0f).endVertex();
		wr.pos(this.posX + w - italicOffset, this.posY + 7.99F, 0.0F).tex(u1f, v1f).endVertex();
		tess.draw();
	...
	public int getCharWidth(char character) {
		if (character == 167) {
			...
			return 4;
		} else if (unicodeFlag && glyphWidth[character] != 0) {
			int var3 = glyphWidth[character] >> 4;
			int var4 = glyphWidth[character] & 15;
			if (var4 > 7) {
				var4 = 15;
				var3 = 0;
			}
	...
	private String bidiReorder(String s) {
		...
		String[] runs = new String[runCount];

		for (int i = 0; i < runCount; ++i) {
			int start = bidi.getRunStart(i);
			int limit = bidi.getRunLimit(i);
			int level = bidi.getRunLevel(i);
			runs[i] = s.substring(start, limit);
			levels[i] = (byte) level;
		}

		String[] copy = runs.clone();
		...
		for (int i = 0; i < runs.length; ++i) {
			...
			if ((level & 1) == 0) {
				...
				for (int j = runs[i].length() - 1; j >= 0; --j) {
					char c = runs[i].charAt(j);
					if (c == 40) {
						c = 41;
					} else if (c == 41) {
						c = 40;

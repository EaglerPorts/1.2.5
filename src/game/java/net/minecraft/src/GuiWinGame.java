...
public class GuiWinGame extends GuiScreen {
	...
	private void respawnPlayer() {
		if (this.mc.theWorld.isRemote) {
			...
			var1.sendQueue.addToSendQueue(new Packet9Respawn(var1.dimension, (byte) this.mc.theWorld.difficultySetting,
					this.mc.theWorld.getWorldInfo().getTerrainType(), this.mc.theWorld.getHeight(), 0));
		} else {
		...
		if (this.lines == null) {
			...
			try {
				...
				BufferedReader var4 = new BufferedReader(
						new InputStreamReader(TexturePack.getResourceAsStream("/title/win.txt"), Charset.forName("UTF-8")));
				...
				while (true) {
					...
					if (var1 == null) {
						...
						var4 = new BufferedReader(new InputStreamReader(TexturePack.getResourceAsStream("/title/credits.txt"),
					...
					String var7;
					String var8;
					for (var1 = var1.replaceAll("PLAYERNAME", this.mc.session.username); var1.indexOf(var2) >= 0; var1 = var7
							+ "\u00a7f\u00a7k" + "XXXXXXXX".substring(0, var5.nextInt(4) + 3) + var8) {
		...
		for (var10 = 0; var10 < this.lines.size(); ++var10) {
			...
			if ((float) var9 + var8 + 12.0F + 8.0F > 0.0F && (float) var9 + var8 < (float) this.height) {
				...
				if (var12.startsWith("[C]")) {
					this.fontRenderer.drawStringWithShadow(var12.substring(3),
							var6 + (var5 - this.fontRenderer.getStringWidth(var12.substring(3))) / 2, var9, 16777215);
				} else {
					this.fontRenderer.fontRandom.setSeed((long) var10 * 4238972211L + (long) (this.updateCounter / 4));
					this.fontRenderer.drawString(var12, var6 + 1, var9 + 1, 16777215, true);
					this.fontRenderer.fontRandom.setSeed((long) var10 * 4238972211L + (long) (this.updateCounter / 4));
					this.fontRenderer.drawString(var12, var6, var9, 16777215, false);
				}
			}

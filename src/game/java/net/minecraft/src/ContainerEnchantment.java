...
public class ContainerEnchantment extends Container {
	...
	public boolean enchantItem(EntityPlayer var1, int var2) {
		...
		if (this.enchantLevels[var2] > 0 && var3 != null
				&& (var1.experienceLevel >= this.enchantLevels[var2] || var1.capabilities.isCreativeMode)) {
			if (!this.worldPointer.isRemote) {
	...
	public boolean canInteractWith(EntityPlayer var1) {
		return this.worldPointer.getBlockId(this.posX, this.posY, this.posZ) != Block.enchantmentTable.blockID ? false
				: var1.getDistanceSq((double) this.posX + 0.5D, (double) this.posY + 0.5D, (double) this.posZ + 0.5D) <= 64.0D;
	}

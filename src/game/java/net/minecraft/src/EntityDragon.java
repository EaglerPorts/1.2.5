...
public class EntityDragon extends EntityDragonBase {
	...
	public EntityDragonPart[] dragonPartArray = new EntityDragonPart[] {
	...
	protected void entityInit() {
		super.entityInit();
		this.dataWatcher.addObject(16, Integer.valueOf(this.maxHealth));
	}
	...
	public void onLivingUpdate() {
		...
		if (this.health <= 0) {
			...
			this.worldObj.spawnParticle("largeexplode", this.posX + (double) var1, this.posY + 2.0D + (double) var26,
			...
			if (this.worldObj.isRemote) {
				...
				if (this.field_40163_ay || var8 < 100.0D || var8 > 22500.0D || this.isCollidedHorizontally
				...
				Vec3D var15 = Vec3D.createVector(this.targetX - this.posX, this.targetY - this.posY, this.targetZ - this.posZ)
						.normalize();
				Vec3D var39 = Vec3D.createVector((double) MathHelper.sin(this.rotationYaw * (float) Math.PI / 180.0F),
						this.motionY, (double) (-MathHelper.cos(this.rotationYaw * (float) Math.PI / 180.0F))).normalize();
				...
				this.randomYawVelocity = (float) ((double) this.randomYawVelocity
						+ var13 * ((double) 0.7F / var19 / (double) var18));
			...
			var26 = (float) (this.func_40160_a(5, 1.0F)[1] - this.func_40160_a(10, 1.0F)[1]) * 10.0F / 180.0F
					* (float) Math.PI;
			...
			this.dragonPartHead.setLocationAndAngles(this.posX + (double) (var31 * 5.5F * var3),
					this.posY + (var9[1] - var29[1]) * 1.0D + (double) (var27 * 5.5F), this.posZ - (double) (var33 * 5.5F * var3),
					0.0F, 0.0F);
			...
			if (!this.worldObj.isRemote) {
				this.field_40161_az = this.destroyBlocksInAABB(this.dragonPartHead.boundingBox)

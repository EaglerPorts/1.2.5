...
public class EntityEnderman extends EntityMob {
	...
	private boolean shouldAttackPlayer(EntityPlayer var1) {
		...
		if (var2 != null && var2.itemID == Block.pumpkin.blockID) {
			...
			Vec3D var4 = Vec3D.createVector(this.posX - var1.posX,
					this.boundingBox.minY + (double) (this.height / 2.0F) - (var1.posY + (double) var1.getEyeHeight()),
					this.posZ - var1.posZ);
	...
	public void onLivingUpdate() {
		...
		if (this.worldObj.isDaytime() && !this.worldObj.isRemote) {
			...
			if (var6 > 0.5F && this.worldObj.canBlockSeeTheSky(MathHelper.floor_double(this.posX),
	...
	protected boolean teleportToEntity(Entity var1) {
		Vec3D var2 = Vec3D.createVector(this.posX - var1.posX,
				this.boundingBox.minY + (double) (this.height / 2.0F) - var1.posY + (double) var1.getEyeHeight(),
				this.posZ - var1.posZ);
		var2 = var2.normalize();
		double var3 = 16.0D;

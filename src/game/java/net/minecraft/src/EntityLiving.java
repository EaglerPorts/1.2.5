...
public class EntityLiving extends Entity {
	...
	public boolean isWithinHomeDistance(int var1, int var2, int var3) {
		return this.maximumHomeDistance == -1.0F ? true
				: this.homePosition.getDistanceSquared(var1, var2, var3) < this.maximumHomeDistance * this.maximumHomeDistance;
	}
	...
	public boolean canEntityBeSeen(Entity var1) {
		return this.worldObj.rayTraceBlocks(
				...
				Vec3D.createVector(var1.posX, var1.posY + (double) var1.getEyeHeight(), var1.posZ)) == null;
	}
	...
	public void onEntityUpdate() {
		...
		if (this.isEntityAlive() && this.isInsideOfMaterial(Material.water) && !this.canBreatheUnderwater()
				&& !this.activePotionsMap.containsKey(Integer.valueOf(Potion.waterBreathing.id))) {
			...
			if (this.getAir() == -20) {
				...
				for (int var1 = 0; var1 < 8; ++var1) {
					...
					this.worldObj.spawnParticle("bubble", this.posX + (double) var2, this.posY + (double) var3,
							this.posZ + (double) var4, this.motionX, this.motionY, this.motionZ);
				}
	...
	public void spawnExplosionParticle() {
		for (int var1 = 0; var1 < 20; ++var1) {
			...
			double var8 = 10.0D;
			this.worldObj.spawnParticle("explode",
					this.posX + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width - var2 * var8,
					this.posY + (double) (this.rand.nextFloat() * this.height) - var4 * var8,
					this.posZ + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width - var6 * var8, var2,
					var4, var6);
		}
			...
			if (this.health <= 0) {
				...
				if (var3) {
					...
					if (var4 != null) {
						...
						this.attackedAtYaw = (float) (Math.atan2(var7, var9) * 180.0D / (double) ((float) Math.PI))
								- this.rotationYaw;
	...
	protected void fall(float var1) {
		...
		if (var2 > 0) {
			...
			if (var3 > 0) {
				...
				this.worldObj.playSoundAtEntity(this, var4.getStepSound(), var4.getVolume() * 0.5F,
						var4.getPitch() * (12.0F / 16.0F));
			}
		}
	...
	public void moveEntityWithHeading(float var1, float var2) {
		...
		if (this.isInWater()) {
			...
			if (this.isCollidedHorizontally && this.isOffsetPositionInLiquid(this.motionX,
					this.motionY + (double) 0.6F - this.posY + var3, this.motionZ)) {
	...
	public void onLivingUpdate() {
		...
		if (this.newPosRotationIncrements > 0) {
			...
			List var9 = this.worldObj.getCollidingBoundingBoxes(this,
					this.boundingBox.contract(1.0D / 32.0D, 0.0D, 1.0D / 32.0D));
			if (var9.size() > 0) {
				double var10 = 0.0D;
		...
		if (this.rand.nextBoolean()) {
			...
			if (var9 > 0) {
				...
				this.worldObj.spawnParticle("mobSpell", this.posX + (this.rand.nextDouble() - 0.5D) * (double) this.width,
						...
						this.posZ + (this.rand.nextDouble() - 0.5D) * (double) this.width, var10, var5, var7);
			}
		}
	...
	public void renderBrokenItemStack(ItemStack var1) {
		...
		for (int var2 = 0; var2 < 5; ++var2) {
			Vec3D var3 = Vec3D.createVector(((double) this.rand.nextFloat() - 0.5D) * 0.1D, Math.random() * 0.1D + 0.1D,
					0.0D);
			var3.rotateAroundX(-this.rotationPitch * (float) Math.PI / 180.0F);
			var3.rotateAroundY(-this.rotationYaw * (float) Math.PI / 180.0F);
			Vec3D var4 = Vec3D.createVector(((double) this.rand.nextFloat() - 0.5D) * 0.3D,
			...
			this.worldObj.spawnParticle("iconcrack_" + var1.getItem().shiftedIndex, var4.xCoord, var4.yCoord, var4.zCoord,

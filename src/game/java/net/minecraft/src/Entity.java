...
public abstract class Entity {
	...
	public void onEntityUpdate() {
		...
		if (this.isSprinting() && !this.isInWater()) {
			...
			if (var4 > 0) {
				this.worldObj.spawnParticle("tilecrack_" + var4,
						this.posX + ((double) this.rand.nextFloat() - 0.5D) * (double) this.width, this.boundingBox.minY + 0.1D,
						this.posZ + ((double) this.rand.nextFloat() - 0.5D) * (double) this.width, -this.motionX * 4.0D, 1.5D,
						-this.motionZ * 4.0D);
			}
		}
		...
		if (this.handleWaterMovement()) {
			if (!this.inWater && !this.firstUpdate) {
				float var6 = MathHelper.sqrt_double(this.motionX * this.motionX * (double) 0.2F + this.motionY * this.motionY
						+ this.motionZ * this.motionZ * (double) 0.2F) * 0.2F;
				if (var6 > 1.0F) {
					var6 = 1.0F;
				}

				this.worldObj.playSoundAtEntity(this, "random.splash", var6,
				...
				for (var3 = 0; (float) var3 < 1.0F + this.width * 20.0F; ++var3) {
					...
					this.worldObj.spawnParticle("bubble", this.posX + (double) var8, (double) (var7 + 1.0F),
				...
				for (var3 = 0; (float) var3 < 1.0F + this.width * 20.0F; ++var3) {
					var8 = (this.rand.nextFloat() * 2.0F - 1.0F) * this.width;
					var5 = (this.rand.nextFloat() * 2.0F - 1.0F) * this.width;
					this.worldObj.spawnParticle("splash", this.posX + (double) var8, (double) (var7 + 1.0F),
	...
	public void moveEntity(double var1, double var3, double var5) {
		if (this.noClip) {
			...
			if (var18) {
				...
				while (var1 != 0.0D && var5 != 0.0D && this.worldObj
						.getCollidingBoundingBoxes(this, this.boundingBox.getOffsetBoundingBox(var1, -1.0D, var5)).size() == 0) {
					if (var1 < var19 && var1 >= -var19) {
						var1 = 0.0D;
					} else if (var1 > 0.0D) {
						var1 -= var19;
	...
	public boolean handleWaterMovement() {
		return this.worldObj.handleMaterialAcceleration(
				this.boundingBox.expand(0.0D, (double) -0.4F, 0.0D).contract(0.001D, 0.001D, 0.001D), Material.water, this);
	}
	...
	public void setPositionAndRotation2(double var1, double var3, double var5, float var7, float var8, int var9) {
		...
		this.setRotation(var7, var8);
		List var10 = this.worldObj.getCollidingBoundingBoxes(this,
